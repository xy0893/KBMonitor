    ...
    while True:
        ...
        if info:
            ...
            if info[1]=='txt' and info[0]:
                write_txt_to(info[0], pathlist['open_filename'], ADDRlist['set_code'],ADDRlist['txtSIZE'],
                        ADDRlist['clientSocket'])
            elif info[1]=='pic' and info[0] and info[2]:
                write_pic_to(info[0], pathlist['open_pic_dir'], info[2], ADDRlist['set_code'], ADDRlist['txtSIZE'],
                        ADDRlist['clientSocket'])
            else:
                print('error >>>',info)
        else:
...
def init_setting():
    ...
    #--------------发包收包编码和BUFSIZE(缓存大小)--------------------------
    ADDRlist['picSIZE'],ADDRlist['txtSIZE']=4096,1024 #图片为4096,文本为1024
